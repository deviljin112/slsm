...
export async function serviceAction(action, service, selectedFile) {
  ...
  for (const out of outputPaths) {
    if (out.includes(service)) {
      ...
      await runCmdInPath(cmd(action), out, fileSpinner);
      installed = true;
    }
  }
  if (installed) {
...
const runCmdInPath = async (cmd, commandPath, fileSpinner) => {
  try {
    await execaCommand(cmd, { cwd: commandPath });
  } catch (err) {
    console.error(err);
    fileSpinner.fail("Failed to run command in path.");
    process.exit(1);
  }
};
